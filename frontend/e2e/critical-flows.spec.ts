...
const teacherPassword =
  process.env.E2E_TEACHER_PASSWORD ?? "arena-dev-change-me";

async function expectOk(response: APIResponse, operation: string) {
  if (!response.ok()) {
    throw new Error(
      `${operation} falhou (${response.status()}): ${await response.text()}`,
    );
  }
}
...
async function createSessionFixture(
  ...
  const enrollment = await api.post(
    `/api/classrooms/${classroom.id}/students/${student.id}`,
    { headers: csrfHeaders },
  );
  ...
  const joinCodeResponse = await api.get(
    `/api/sessions/${session.id}/join-code`,
  );
...
test("professor entra na Arena e participante reconecta à sessão", async ({
  browser,
  page,
}) => {
  ...
  try {
    ...
    await expect(
      page.getByRole("heading", { name: fixture.classroomName }),
    ).toBeVisible();
    await expect(page.getByTitle("Trocar turma")).toContainText(
      fixture.classroomName,
    );
    await page.getByRole("button", { name: "Continuar Arena" }).click();
    await expect(
      page.getByText("Arena", { exact: true }).first(),
    ).toBeVisible();
    await expect(
      page.getByText(fixture.sessionTitle, { exact: true }),
    ).toBeVisible();
    ...
    try {
      ...
      await expect(
        participantPage.getByRole("heading", { name: fixture.studentNickname }),
      ).toBeVisible();
      await expect(
        participantPage.getByText("Conectado", { exact: true }),
      ).toBeVisible();
    } finally {
...
test("roteiro preparado sincroniza slide, questão, nuvem, votação e timeline", async ({
  browser,
}) => {
  const { api, csrfHeaders } = await authenticatedApi();
  const participantContext = await browser.newContext();
  const participantPage = await participantContext.newPage();
  let projectorContext:
    | Awaited<ReturnType<typeof openProjector>>["context"]
    | undefined;
  ...
  try {
    ...
    const stepsResponse = await api.put(
      `/api/activities/${activity.id}/steps`,
      {
        headers: csrfHeaders,
        data: [
          {
    ...
    const arenaResponse = await api.put(
      `/api/sessions/${fixture.session.id}/mechanics/arena`,
      {
        headers: csrfHeaders,
    ...
    const start = await api.post(
      `/api/sessions/${fixture.session.id}/mechanics/arena/flow/start`,
      { headers: csrfHeaders },
    );
    await expectOk(start, "Início do Live Flow E2E");
    await expect(
      projector.page.getByRole("heading", {
        level: 1,
        name: slideTitle,
        exact: true,
      }),
    ...
    const question = await api.post(
      `/api/sessions/${fixture.session.id}/mechanics/arena/flow/next`,
      { headers: csrfHeaders },
    );
    await expectOk(question, "Avanço para questão E2E");
    await expect(
      projector.page.getByText(questionText, { exact: true }),
    ).toBeVisible();
    await expect(
      participantPage.getByText(questionText, { exact: true }),
    ).toBeVisible();

    const cloud = await api.post(
      `/api/sessions/${fixture.session.id}/mechanics/arena/flow/next`,
      { headers: csrfHeaders },
    );
    await expectOk(cloud, "Avanço para Nuvem E2E");
    await expect(
      participantPage.getByText(cloudPrompt, { exact: true }),
    ).toBeVisible();
    await expect(
      projector.page.getByText(cloudPrompt, { exact: true }),
    ).toBeVisible();
    await participantPage
      .getByPlaceholder("Sua palavra ou expressão")
      .fill(cloudWord);
    await participantPage.getByRole("button", { name: "Enviar" }).click();
    await expect(
      participantPage.getByText(cloudWord, { exact: true }),
    ).toBeVisible();

    const cloudStateResponse = await api.get(
      `/api/sessions/${fixture.session.id}/word-cloud`,
    );
    ...
    const revealCloud = await api.post(
      `/api/sessions/${fixture.session.id}/word-cloud/${cloudState.round!.id}/reveal`,
      { headers: csrfHeaders },
    );
    await expectOk(revealCloud, "Reveal da Nuvem E2E");
    await expect(
      projector.page.getByText(cloudWord, { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      participantPage.getByText(pollPrompt, { exact: true }),
    ).toBeVisible();
    await expect(
      projector.page.getByText(pollPrompt, { exact: true }),
    ).toBeVisible();
    await participantPage
      .getByRole("button", { name: new RegExp(pollYes) })
      .click();
    await expect(
      participantPage.getByText(
        "Seu voto foi registrado. Você pode acompanhar o estado desta rodada aqui.",
      ),
    ).toBeVisible();

    const pollStateResponse = await api.get(
      `/api/sessions/${fixture.session.id}/poll`,
    );
    ...
    const revealPoll = await api.post(
      `/api/sessions/${fixture.session.id}/poll/${pollState.round!.id}/reveal`,
      { headers: csrfHeaders },
    );
    ...
    const eventsResponse = await api.get(
      `/api/sessions/${fixture.session.id}/events`,
    );
...
test("Buzzer e Boss permanecem sincronizados entre participante e Projetor", async ({
  browser,
}) => {
  const { api, csrfHeaders } = await authenticatedApi();
  const participantContext = await browser.newContext();
  ...
  try {
    ...
    const openBuzzer = await api.post(
      `/api/sessions/${fixture.session.id}/buzzer/open`,
      { headers: csrfHeaders },
    );
    await expectOk(openBuzzer, "Abertura do Buzzer E2E");
    await expect(
      participantPage.getByRole("heading", { name: "Valendo!" }),
    ).toBeVisible();
    await participantPage.getByRole("button", { name: "APERTAR" }).click();
    await expect(
      participantPage.getByRole("heading", { name: "Você é o 1º" }),
    ).toBeVisible();
    await expect(
      projector.page.getByText(fixture.studentNickname, { exact: true }),
    ).toBeVisible();

    const closeBuzzer = await api.post(
      `/api/sessions/${fixture.session.id}/buzzer/close`,
      { headers: csrfHeaders },
    );
    await expectOk(closeBuzzer, "Fechamento do Buzzer E2E");

    const bossName = `Legacy Monolith ${fixture.suffix.slice(-6)}`;
    const startBoss = await api.post(
      `/api/sessions/${fixture.session.id}/mechanics/boss`,
      {
        headers: csrfHeaders,
    ...
    await expect(
      projector.page.getByText(bossName, { exact: true }),
    ).toBeVisible();
    await expect(
      participantPage.getByText(bossName, { exact: true }),
    ).toBeVisible();
    await expect(
      projector.page.getByText("10 HP", { exact: true }),
    ).toBeVisible();

    const defeatBoss = await api.post(
      `/api/sessions/${fixture.session.id}/mechanics/boss/damage`,
      {
        headers: csrfHeaders,
        data: { amount: 10 },
      },
...
test("Quiz completo preserva resposta no reconnect, revela no Projetor e gera ScoreEvent", async ({
  browser,
}) => {
  ...
  try {
        ...
        questions: [
          {
            ...
            expectedAnswer: null,
            explanation:
              "A alternativa A é a resposta autorada para o gate E2E.",
            code: null,
            language: null,
            expectedOutcome: null,
            evaluationCriteria: [],
    ...
    const prepareResponse = await api.post(
      `/api/sessions/${fixture.session.id}/quiz`,
      {
        headers: csrfHeaders,
        data: { questionId },
      },
    ...
    await participantPage
      .getByRole("radio", { name: new RegExp(correctText) })
      .click();
    await expect(
    ...
    await projectorContext?.close().catch(() => {});
